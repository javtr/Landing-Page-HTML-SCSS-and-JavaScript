<?php

require_once __DIR__.'/../vendor/autoload.php';
require_once __DIR__.'/config.php';

session_start();

if (!empty($_SESSION['_form_form_error'])) {
    $error = $_SESSION['_form_form_error'];
    unset($_SESSION['_form_form_error']);
}

if (!empty($_SESSION['_form_form_success'])) {
    $success = true;
    unset($_SESSION['_form_form_success']);
}

?>
<!doctype html>
<html lang="en">


<head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Purchase Form - Logic Indicators</title>
    <link rel="stylesheet" href="../../scss/style.css" />
    <link rel="stylesheet" href="../../scss/form.css">
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
      href="https://fonts.googleapis.com/css2?family=Open+Sans:wght@300;400;700&display=swap"
      rel="stylesheet"
    />

    <script src="https://www.google.com/recaptcha/api.js" async defer></script>
    <link rel="icon" href="../../../images/icono.ico">

  </head>


<body>


    <!-- ====================== form =========================== -->

    <section class="form">
      <div class="container container--contact">

      <?php
            if (!empty($success)) 
            {
                    ?>
                <div class="sucess">
                    <a class="sucess-element">your message has been sent! <br> we will soon send your Logic software to your mailbox.</a>
                </div>
                <?php
            }
        ?>

        <?php
            if (!empty($error))
             {
                    ?>
                <div class="error">
                    <div class="error-element"><?= $error ?></div>
                    <a class="error-element">Your message could not be sent <br> If the error persists, please contact contact@logic.com. </a>
                </div>                        
                <?php
            }
        ?>



      <div class="form__logo">
            <img src="../../../images/logo_logic04.svg" alt="">
        </div>







        <div class="form__card">

            <div class="form__header">
                <p>Do not close this window until your process is complete!</p>
            </div>



            <div class="form__title">Thank you for purchasing <br> Logic Indicators</div>

            <div class="form__subtitle">
                <p>Follow carefully the following steps to get your software</p>
            </div>



            <div class="form__numeral">
                <p>1. Get the Machine ID of your ninjatrader</p>
            </div>

            <div class="form__image">
                <img src="../../../images/id.jpg" alt="" srcset="">
            </div>

            <div class="form__image--text">
                <p>Machine ID code is generated by NinjaTrader based on the main system hardware and software.</p>
            </div>




            <div class="form__numeral2">
                <p>2. After submitting this form, we will create the license for your pcg.</p>
            </div>


            <form method="post" action="submit.php" class="contact-form">


                <div class="form__label-form">Name <span>*</span> </div>
                <input type="text" name="name" id="name" class="form__input-form" placeholder="John Doe">

                <div class="form__label-form">Email <span>*</span> </div>
                <input type="email" name="email" id="email" class="form__input-form" placeholder="form@mail.com">

                <div class="form__label-form">Machine ID <span>*</span> </div>
                <input type="text" name="idmachine" id="idmachine" class="form__input-form" placeholder="1234123412341234">


                <div class="form__label-form">Message</div>
                <textarea type="text" name="message" id="message" class="form__input-form-text" placeholder=""></textarea>
            
                <div class="form-group text-center form__captcha">
                    <div class="g-recaptcha form__captcha--box" data-sitekey="<?= CONTACTFORM_RECAPTCHA_SITE_KEY ?>"></div>
                  </div>


                <button class="btn btn-primary btn-block btn button-contact">Send</button>
        
           
            </form>



            <div class="form__note">
                <p>• You will receive your software in your mailbox within 1 business day. (8 - 5 PM GMT-4).</p>
            </div>

        </div>
      </div>
    </section>




    <script src="../../js/script.js"></script>
</body>
</html>
